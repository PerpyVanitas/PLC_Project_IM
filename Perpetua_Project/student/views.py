...
from login.models import Department,Admin,Class,Mentee,Faculty,Calendar,Course,Attendance,Timetable,Mentor
...
def studlogin(request):
    if request.method=="POST":
        ...
        stud=Mentee.objects.filter(stud_id=u)
        if stud.exists():
...
def studprofile(request):
    if request.method=="POST":
        try:
            stud=Mentee.objects.filter(stud_id=stu)
            fn=request.POST.get('fn')
            ln=request.POST.get('ln')
            pa=request.POST.get('pass')
            if fn != "":
                Mentee.objects.filter(stud_id=stu).update(f_name=fn)
            if ln !="" :
                Mentee.objects.filter(stud_id=stu).update(l_name=ln)
            if pa !="" :
                Mentee.objects.filter(stud_id=stu).update(f_password=pa)
        except:
...
def stud_report(request):
    ...
    stud=Mentee.objects.all().filter(stud_id=stu)
